...
const non_replacement_random_item_sampler_1 = require("non-replacement-random-item-sampler");
...
const createPopulatedTestCache = () => {
    const cache = new fifo_ttl_cache_1.FIFOCache(MOCK_CAPACITY, MOCK_TTL_MS);
...
const createAllKeys = () => new Array(MOCK_CAPACITY).fill(0).map((_, index) => index + 1);
describe('FIFOCache tests', () => {
    ...
    describe('Happy path tests', () => {
        ...
        test('values should be updated successfully', () => {
            ...
            const uniqueKeySampler = new non_replacement_random_item_sampler_1.NonReplacementRandomSampler(allKeys);
    ...
    describe('Negative path tests', () => {
        test('constructor should throw an error if the _capacity is not a natural number', () => {
            ...
            for (const invalidCapacity of nonNaturalNumbers) {
                expect(() => new fifo_ttl_cache_1.FIFOCache(invalidCapacity, MOCK_TTL_MS)).toThrow();
            }
        });
        test('constructor should throw an error if the _ttlMs is not a natural number', () => {
            const nonNaturalNumbers = [-74, -65, -5.67, -0.00001, 0, 0.1, 0.08974, 9.543, 1898.5, 4000.0000001];
            for (const invalidTTL of nonNaturalNumbers) {
                expect(() => new fifo_ttl_cache_1.FIFOCache(MOCK_CAPACITY, invalidTTL)).toThrow();
            }
        });
    });
});
//# sourceMappingURL=fifo-ttl-cache.test.js.map